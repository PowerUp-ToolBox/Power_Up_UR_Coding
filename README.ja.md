...
変更の承認、ターンの中断、直前の返答のもう一度再生、定型プロンプトの送信——
コントローラーのどのボタンにも、ワークフローに合わせて好きな操作を割り当て
られます。送信前に文面を確認したい？ もう一方のトリガーを押しながらプロンプト
欄へ音声入力し、納得してから編集して送ればOK。ライトバーとハプティクスが
セッションの状態を映すので、いま待機中か、聞き取り中か、思考中か、読み上げ中か
はひと目で分かります。
...
Gemini CLI、opencode などの**どの AI コーディングエージェント**でも、macOS・
Windows・Linux の**どの OS**でも、ハンズフリーでコードを書けるようにする。
しかもデバイス側のライト・振動・画面が、エージェントのいまの動きをリアルタイム
に知らせてくれる——それがゴールです。

全体計画は **[DEVELOPMENT.md](DEVELOPMENT.md)**、参加のしかたは
**[CONTRIBUTING.md](CONTRIBUTING.md)** へ。コントリビューション、対応デバイス
の提案、対応ツールのリクエスト、どれも歓迎です。issue か discussion を
立ててください。
...
| [ボタンと操作](docs/controls.md) | デフォルト割り当ての全一覧、音声入力→確認→送信、モデル/思考の深さ/権限の切り替え、中断 |
